...
Shader "Custom/FastLineRendererShader"
{
    ...
    SubShader
	{
		...
		inline fixed4 lerpFade(float4 lifeTime, float timeSinceLevelLoad)
		{
			...
			if (timeSinceLevelLoad < peakFadeIn)
			{
				return lerp(fixed4(0, 0, 0, 0), fixed4(1, 1, 1, 1), max(0, ((timeSinceLevelLoad - lifeTime.x) / (peakFadeIn - lifeTime.x))));
			}
			float endLifetime = (lifeTime.x + lifeTime.z);
			float startFadeOut = endLifetime - lifeTime.y;

			return lerp(fixed4(1, 1, 1, 1), fixed4(0, 0, 0, 0), max(0, ((timeSinceLevelLoad - startFadeOut) / (endLifetime - startFadeOut))));
		}
